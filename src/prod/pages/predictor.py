...
if uploaded_file is not None:
    ...
    dataframe.dropna(subset=['w_ace', 'w_df', 'w_svpt', 'w_1stIn', 'w_1stWon'], inplace=True)
    dataframe["winner_ht"].fillna(dataframe["winner_ht"].mean(), inplace=True)
    dataframe["loser_ht"].fillna(dataframe["loser_ht"].mean(), inplace=True)
    dataframe["loser_age"].fillna(dataframe["loser_age"].mean(), inplace=True)
    dataframe["minutes"].fillna(dataframe["minutes"].mean(), inplace=True)

    #Escalado de datos
    columns_LE = ["tourney_name", "winner_ioc", "loser_ioc", "round"]
    le = LabelEncoder()

    dataframe["tourney_name"] = le.fit_transform(dataframe["tourney_name"])
    dataframe["winner_ioc"] = le.fit_transform(dataframe["winner_ioc"])
    dataframe["loser_ioc"] = le.fit_transform(dataframe["loser_ioc"])
    ...
    dataframe["surface"].replace(map_surface, inplace=True)
    dataframe["loser_hand"].replace(map_hand, inplace=True)
    dataframe["winner_hand"].replace(map_hand, inplace=True)
    dataframe["tourney_level"].replace(map_tourney, inplace=True)
